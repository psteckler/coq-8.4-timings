...
let rewrite_with flags c left2right loccs : strategy =
  fun env avoid t ty cstr evars ->
    ...
    let _ = Printf.printf "about to rewrite with %s\n%!" (Pp.string_of_ppcmds (Printer.pr_constr_env env !hypinfo.prf)) in
...
let get_hyp gl evars (c,l) clause l2r =
  let _ = Printf.printf "Entering: %s\n%!" (Pp.string_of_ppcmds (Printer.pr_constr c)) in
